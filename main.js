...
// Wayland screen capture goes through the PipeWire portal; without this
// feature flag desktopCapturer on Linux may return no sources.
...
let win = null;
// Which global shortcuts cirax actually holds. `globalShortcut.register` returns
...
function createWindow() {
  ...
  // On Linux, Electron's setContentProtection is a no-op today — we still call it
  // (harmless, future-proof) but tell the user the truth instead of promising stealth.
  const shouldProtect = !process.env.CIRAX_NO_PROTECT;
  if (shouldProtect) {
    if (WIN_SUPPORTS_CONTENT_PROTECTION) {
      win.setContentProtection(true);
    } else {
      // Will notify the renderer after it loads
      console.log(`[cirax] Windows build ${WIN_BUILD} < 19041 — setContentProtection not supported. Window may appear in screen shares.`);
    }
  }
...
function handleSttError(err, settings) {
  ...
  // occurrence still tell you the state cirax is stuck in.
  recordEvent({
    level: 'error',
    event: 'stt_rejected',
...
// getDisplayMedia loopback for system audio) so they run inside cirax's own process
// and use cirax's own Screen-Recording grant — no separate helper binary to authorize.
  ...
  if (active) {
    ...
    if ((settings.sttProvider || 'auto') === 'local') {
      try {
        ...
        console.log('[cirax] capture started, mode: local');
    ...
    console.log('[cirax] capture started, mode:', streaming ? 'streaming' : 'batch');
...
async function runFeature(mode, userText) {
  ...
  try {
    ...
    if (def.needsScreen) {
      try {
        imageDataUrl = await captureScreenshot({ mode: settings.captureMode || 'screen' });
        ...
        const message = process.platform === 'darwin'
          ? 'Screen capture needs permission — grant Screen Recording to cirax in System Settings.'
    ...
    send('llm:done', {});
    try {
      const { getSessionUsage } = require('./src/llm');
      send('usage', getSessionUsage());
...
function launchApp() {
  ...
  // audio so the renderer can capture what's playing (Zoom/Meet) using cirax's own grant.
