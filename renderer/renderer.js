/* cirax renderer — UI state, mic capture, IPC, streaming render. */
(function () {
  const { icon } = window.ICONS;
  const cirax = window.cirax; // exposed by preload
  const $ = (s) => document.querySelector(s);
  const isWindows = cirax.platform === 'win32';
  const isMac = cirax.platform === 'darwin';
  ...
  smartBtn.addEventListener('click', async () => {
    ...
    await cirax.settingsSet({ smart: settings.smart });
  });
  ...
  async function startMic() {
    ...
    try {
      ...
      // "cirax never hears me, no error shown" symptom with no diagnostic at all.
      ...
      cirax.log('mic stream started: track=' + (track.label || '(no label — permission may be stale)') + ' muted=' + track.muted);
      ...
      try {
        ...
        cirax.log('AudioWorklet failed, falling back to ScriptProcessor: ' + workletErr.message);
      ...
      cirax.log('mic error: ' + name + ' — ' + message);
      ...
      if (name === 'NotFoundError' || name === 'DevicesNotFoundError') {
        ...
        showStatus(isWindows
          ? 'Microphone permission was denied. Settings → Privacy & security → Microphone → allow cirax, then try again.'
          : 'Microphone permission was denied. System Settings → Privacy & Security → Microphone → allow cirax, then try again.');
  ...
  async function startSystemAudio() {
    ...
    try {
      ...
      if (!tracks.length) {
        cirax.log('system audio: no loopback track on this platform');
        stream.getTracks().forEach((t) => t.stop());
        showStatus(cirax.platform === 'win32'
      ...
      try {
        ...
        sysWorklet = new AudioWorkletNode(sysCtx, 'cirax-audio-processor');
        ...
        cirax.log('system audio AudioWorklet failed, using ScriptProcessor: ' + workletErr.message);
        ...
        sysProc.onaudioprocess = (e) => {
          ...
          const out = new Int16Array(f.length);
          for (let i = 0; i < f.length; i++) { const s = Math.max(-1, Math.min(1, f[i])); out[i] = s < 0 ? s * 0x8000 : s * 0x7fff; }
          cirax.systemPcm(out.buffer);
        };
        sysWorklet = { _legacy: true, proc: sysProc, node: sysNode, sink };
      }
    } catch (err) {
      const message = err && err.message ? err.message : String(err);
      cirax.log('system audio error: ' + message);
      showStatus('Meeting audio could not be started. Grant screen/audio access to cirax and try again.');
    } finally {
      sysStarting = false;
    }
  }
  ...
  // ---- events from main --------------------------------------------------
  cirax.on('capture:state', ({ active, streaming, mode }) => {
  ...
  cirax.on('stt:interim', ({ channel, text }) => {
  ...
  cirax.on('stt:final', ({ channel, text }) => {
  ...
  cirax.on('usage', (u) => {
    ...
    if (!el) return;
    el.textContent = `tokens ↑${u.promptTokens} ↓${u.completionTokens} · ${u.requests} requests`;
  });
  cirax.on('stt:status', ({ channel, status, provider }) => {
    cirax.log(`[stt] ${provider || channel || 'unknown'} ${status}`);
    if (provider === 'local') {
      const label = document.getElementById('stt-status');
      const localLabels = {
  ...
  cirax.on('vad:state', ({ channel, speaking }) => {
    setLiveDotState(speaking ? 'speaking' : 'idle');
  });
  cirax.on('llm:start', ({ userBubble, small, category }) => {
    responseCount++;
  ...
  cirax.on('llm:token', ({ text }) => appendToken(text));
  cirax.on('llm:done', () => { finalizeAi(); setBusy(false); });
  cirax.on('llm:error', ({ message }) => {
    if (!aiEl) startAi(true);
    aiEl.dataset.raw = message; finalizeAi(); setBusy(false);
  });
  cirax.on('transcript', ({ channel, text }) => {
  ...
  function showStatus(message) {
    let el = document.getElementById('cirax-status');
    if (!el) {
      el = document.createElement('div');
      el.id = 'cirax-status';
  ...
  cirax.on('status', ({ message }) => {
    cirax.log('[status] ' + message);
    showStatus(message);
  ...
  function showMicPermissionBanner() {
    ...
    banner.innerHTML =
      '<div class="mic-perm-text">' +
        ...
        'cirax needs microphone permission to hear you during calls. Grant access in System Settings, then restart cirax.' +
      '</div>' +
      '<div class="mic-perm-actions"></div>';
    const actions = banner.querySelector('.mic-perm-actions');
    if (cirax.platform === 'darwin') {
      const openBtn = document.createElement('button');
      openBtn.textContent = 'Open Microphone Settings';
      openBtn.addEventListener('click', () => cirax.openPane('x-apple.systempreferences:com.apple.preference.security?Privacy_Microphone'));
  ...
  function fillSettings() {
    ...
    $('#capture-mode').value = settings.captureMode || 'screen';
  ...
  // Whoever cirax has been told it may answer questions for. Empty is the normal
  ...
  async function fillAppLinkCallers() {
    ...
    if (!host || !cirax.appLinkState) return;
    let state;
    try { state = await cirax.appLinkState(); } catch (_) { return; }
  ...
  if (uploadResumeBtn) uploadResumeBtn.addEventListener('click', async () => {
    const res = await cirax.pickProfileDocument();
  ...
  async function refreshWhisperModels() {
    ...
    try {
      ...
      whisperOverview = await cirax.whisperModels();
  ...
  $('#whisper-download').addEventListener('click', async () => {
    ...
    try {
      await cirax.whisperModelDownload(model.id);
  ...
  $('#whisper-cancel').addEventListener('click', async () => {
    const model = getSelectedWhisperModel();
    if (model) await cirax.whisperModelCancel(model.id);
  });
  ...
  $('#whisper-import').addEventListener('click', async () => {
    ...
    try {
      const result = await cirax.whisperModelImport(model.id);
  ...
  $('#whisper-delete').addEventListener('click', async () => {
    ...
    try {
      await cirax.whisperModelDelete(model.id);
  ...
  cirax.on('whisper:download-progress', (progress) => {
  ...
  cirax.on('whisper:models-changed', () => refreshWhisperModels());
  ...
  function answerConsent(allowed) {
    ...
    cirax.appLinkConsentRespond(pendingConsentId, allowed);
    pendingConsentId = null;
    consentScrim.classList.add('hidden');
  }

  cirax.on('applink:consent-request', (request) => {
  ...
  const permissionHelp = isWindows
    ? 'cirax needs permission to see and hear. Open Windows Privacy & security settings, allow <strong>Microphone</strong> and <strong>Screen recording</strong> for cirax, then come back here.'
    : 'cirax needs two macOS permissions. Click each button, turn <strong>cirax</strong> ON in the window that opens, then come back here.';
  const permissionButtons = isWindows
    ? [
        { label: 'Open Microphone settings', action: () => cirax.openPane('ms-settings:privacy-microphone') },
        { label: 'Open Screen recording settings', action: () => cirax.openPane('ms-settings:privacy-screenrecorder') }
      ]
    : [
        { label: 'Open Microphone settings', action: () => cirax.openPane('x-apple.systempreferences:com.apple.preference.security?Privacy_Microphone') },
        { label: 'Open Screen Recording settings', action: () => cirax.openPane('x-apple.systempreferences:com.apple.preference.security?Privacy_ScreenCapture') }
      ];
  ...
  const OB_STEPS = [
    {
      icon: '👋',
      title: 'Welcome to cirax',
      body: 'cirax is a private AI copilot that floats over your screen. It can <strong>see your screen</strong>, <strong>hear your meetings</strong>, and help you answer questions or solve coding problems — while staying hidden from most screen shares.<br><br>This quick guide gets you running in about a minute.'
    },
    {
      icon: '🔐',
      title: 'Allow cirax to see & hear',
  ...
  async function finishOnboard() {
    ...
    if (settings && !settings.onboarded) { settings.onboarded = true; await cirax.settingsSet({ onboarded: true }); }
  }
  ...
  // ---- boot --------------------------------------------------------------
  (async function boot() {
    settings = await cirax.settingsGet();
    const platformInfo = await cirax.platformInfo();
