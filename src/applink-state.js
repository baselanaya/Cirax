// What Iris is allowed to know about cirax.
//
...
function describeState({ state, transcript, settings, sttDisabled, shortcuts, windowAlive }) {
  ...
  return {
    ...
    // The single most useful field here. When this is true cirax looks alive and
 ...
 *
 * On the Node transport cirax cannot read the peer's credentials, so the caller's
...
function consentCopy(request) {
  ...
  return {
    trusted,
    message: action ? `${who} wants to control cirax.` : `${who} wants to see what cirax is doing.`,
    detail:
      (action
        ? 'It would be able to start and stop listening. '
        : 'It would be able to read cirax’s status, recent warnings and errors — never your transcript, your résumé or your API keys. ') +
      (trusted
        ? 'Its code signature has been verified.'
        : 'cirax cannot verify what this program really is; anything running under your account could make the same claim.') +
      '\n\nYou can change this later in cirax’s settings.',
