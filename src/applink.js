// The publik app link: how Iris asks cirax what it is doing.
//
// Before this, an assistant helping someone with cirax could only see what was on
...
//
// So cirax answers questions instead. Nothing is exposed until the user says yes,
...
/**
 * Ask inside cirax's own window.
 *
 * The first version of this used dialog.showMessageBox, and it was unusable:
 * cirax calls app.dock.hide(), so it is an accessory application and never
 ...
 * bringing it forward.
 *
 * cirax's own window has none of that problem, matches the rest of the app, and
...
function askInWindow(win, copy, scope) {
  ...
  return new Promise((resolve) => {
    ...
    // cirax deliberately never steals focus — except here. A question about who
 ...
 *   `get_state` reflects the moment it is asked rather than the moment cirax
 *   started.
 */
function startAppLink(deps) {
  ...
  link = new AppLinkServer({
    appId: 'com.cirax.overlay', // Matches electron-builder.cjs. Read off a build, not guessed.
    appSlug: 'cirax',
    appName: 'cirax',
  ...
  link.start().catch((error) => {
    // A link that will not start must never stop cirax from starting. The app
    // worked without this yesterday.
    console.log('[cirax] app link unavailable:', error && error.message);
    link = null;
  });
