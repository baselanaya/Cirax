...
// Normalized across providers: { prompt_tokens, completion_tokens } shape in,
// { promptTokens, completionTokens } out. Streams that don't report usage
// simply don't add to the counters.
const sessionUsage = { requests: 0, promptTokens: 0, completionTokens: 0, byProvider: {} };

function addUsage(provider, usage) {
  if (!usage) return;
  const pt = usage.prompt_tokens || usage.input_tokens ||
    (usage.metadata && (usage.metadata.prompt_token_count || 0)) || 0;
  const ct = usage.completion_tokens || usage.output_tokens ||
    (usage.metadata && (usage.metadata.candidates_token_count || 0)) || 0;
  if (!pt && !ct) return;
  sessionUsage.requests += 1;
  sessionUsage.promptTokens += pt;
  sessionUsage.completionTokens += ct;
  const k = sessionUsage.byProvider[provider] = sessionUsage.byProvider[provider] ||
    { requests: 0, promptTokens: 0, completionTokens: 0 };
  k.requests += 1; k.promptTokens += pt; k.completionTokens += ct;
}

function getSessionUsage() { return JSON.parse(JSON.stringify(sessionUsage)); }
function resetSessionUsage() {
  sessionUsage.requests = 0; sessionUsage.promptTokens = 0; sessionUsage.completionTokens = 0;
...
module.exports = { createLLM, formatProviderErrorMessage, isQuotaError, CURRENT_GEMINI_DEFAULT, getSessionUsage, resetSessionUsage };
