...
const MODES = {
  ...
  assist: {
    ...
    buildSystem(contextBlock, aiRules) {
      return applyRules(buildSystem(
        'You are cirax, a discreet real-time copilot overlaid on the user\'s screen during an interview or coding session. ' +
        BASE_RULES +
  ...
  say: {
    ...
    buildSystem(contextBlock, aiRules) {
      return applyRules(buildSystem(
        'You are cirax, whispering the perfect reply to the candidate during a live interview. ' +
        BASE_RULES +
  ...
  followup: {
    ...
    buildSystem(contextBlock, aiRules) {
      return applyRules(buildSystem(
        'You are cirax. Suggest 2–4 sharp follow-up questions the candidate could ask the interviewer.\n' +
  ...
  recap: {
    ...
    buildSystem(contextBlock, aiRules) {
      return applyRules(buildSystem(
        'You are cirax. Summarize the interview so far:\n' +
  ...
  ask: {
    ...
    buildSystem(contextBlock, aiRules) {
      return applyRules(buildSystem(
        'You are cirax, a real-time copilot with access to the candidate\'s screen and live interview. ' +
        BASE_RULES +
  ...
  answerThis: {
    ...
    buildSystem(contextBlock, aiRules) {
      return applyRules(buildSystem(
        'You are cirax, whispering a direct answer to the candidate for ONE specific question. ' +
        BASE_RULES +
