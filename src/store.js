...
const FILE = path.join(app.getPath('userData'), 'cirax-data.json');
...
let data = null;
let secretsBackend = 'none'; // 'safeStorage' (encrypted at rest) | 'plain' (no keyring)

function initCrypto() {
  try {
    secretsBackend = safeStorage.isEncryptionAvailable() ? 'safeStorage' : 'plain';
  } catch (e) {
    secretsBackend = 'plain';
  }
}

// Encrypt the runtime apiKeys map. Returns '' when there is nothing to
// protect or no OS keyring — the caller then falls back to plaintext.
function encryptKeys(keys) {
  if (!keys || !Object.keys(keys).some((k) => keys[k])) return '';
  if (secretsBackend !== 'safeStorage') return '';
  return safeStorage.encryptString(JSON.stringify(keys)).toString('base64');
}

function decryptKeys(enc) {
  try {
    if (!enc) return null;
