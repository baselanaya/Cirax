...
test('reports what cirax is doing', () => {
...
/**
 * The one test in this file that matters more than the others. cirax's transcript
...
test('hedges the consent sheet when the caller cannot be verified', () => {
  ...
  assert.equal(verified.message, 'Iris wants to see what cirax is doing.');
...
test('answers Iris over the link', async (t) => {
  ...
  const found = AppLinkClient.discover(pathOptions).find((entry) => entry.appId === 'com.cirax.overlay');
  assert.ok(found, 'cirax did not announce itself');
  ...
  assert.equal(bundle.app.slug, 'cirax');
